...
LSM:Register("font", "test", [[Interface\addons\PlateBuffs\media\Fonts\test.TTF]])
...
defaultSettings.saveNameToGUID = false
defaultSettings.watchCombatlog = false
...
defaultSettings.decimalDisplayedBelowXSeconds = 10 --++13.12.23
defaultSettings.showCooldownText = true --++13.12.23
defaultSettings.showCooldownTextLessThanXSeconds = 10 --++13.12.23
defaultSettings.scanNameplatesInterval = 0.1 --++13.12.23
...
core.CoreOptionsTable = {
	...
	args = {
		...
		defaultBuffShow = {
			...
			order = 3,
			values = {ALL, L["Mine + SpellList"], L["Only SpellList"], L["Mine Only"], "None"}
		},
		defaultDebuffShow = {
			type = "select",
			name = L["Show Debuffs"],
        ...
        ["Interface\\Addons\\PlateBuffs\\media\\Border2.tga"] = "Border2",
        ["Interface\\Addons\\PlateBuffs\\media\\Gloss.tga"] = "Gloss",
        ["Interface\\Addons\\PlateBuffs\\media\\Highlight.tga"] = "Highlight",
        ["Interface\\Addons\\PlateBuffs\\media\\Highlight2.tga"] = "Highlight2",
        ["Interface\\Addons\\PlateBuffs\\media\\Normal.tga"] = "Normal",
        ["Interface\\Addons\\PlateBuffs\\media\\Normal_Vista.tga"] = "Normal_Vista",
        ["Interface\\Addons\\PlateBuffs\\media\\Normal_Vista_2.tga"] = "Normal_Vista_2",
        ["Interface\\Addons\\PlateBuffs\\media\\ThinSquare.tga"] = "ThinSquare",
		...
		showCooldown = {
			type = "toggle",
			name = "Show cooldown text under the spell icon",
		...
		showCooldownTexture = {
			type = "toggle",
			name = "Show cooldown overlay animation",
			order = 25
		},
		...
		additionalSetting = { --++13.12.23
			type = "header",
			name = "|cff3377dd-={ Additional settings }=-|r",
			order = 50
		},
		decimalDisplayedBelowXSeconds = { --++13.12.23
			type = "range",
			name = "|cff3377ddDecimal numbers are displayed when less than X seconds|r",
		...
		showCooldownText = { --++13.12.23
			type = "toggle",
			name = "|cff3377ddShow aura time left text|r",
			desc = "Default: true",
			order = 60,
		},
		showCooldownTextLessThanXSeconds = { --++13.12.23
			type = "range",
			name = "|cff3377ddShow aura time left text only when time less than X seconds|r",
			desc = "Default: 10",
			min = 0,
			max = 10000,
      step = 1,
		...
		showStacks = { --++13.12.23
			type = "toggle",
			name = "|cff3377ddShow stacks text count|r",
      desc = "Default: true",
			order = 70,
		},
		scanNameplatesInterval = { --++13.12.23
			type = "range",
			name = "|cff3377ddScan nameplates interval|r",
			desc = "This value probably needs to be increased to get maximum performance on low PCs, default: ~0.1",
			min = 0.01,
			max = 3,
      step = 0.01,
			order = 75,
		},
		blackBorderForAll = { --++13.12.23
			type = "toggle",
			name = "|cff3377ddBlack border for all auras|r",
      desc = "Default: false",
			order = 80,
		},
    showCdBgTexture = { --++13.12.23
			type = "toggle",
			name = "|cff3377ddShow text background black texture|r",
      desc = "Make it easier to see, when the text is below icon, reload UI is required, default: true",
			order = 85,
		},
    textColoringRedToGreen = { --++13.12.23
			type = "toggle",
			name = "|cff3377ddText coloring|r",
...
-- core.CoreOptionsTable.args.borderTexture.values[""] = NONE
-- core.CoreOptionsTable.args.borderTexture.values["Interface\\Addons\\PlateBuffs\\media\\border.tga"] = "Default"
-- core.CoreOptionsTable.args.borderTexture.values["Masque"] = (MSQ and "Masque" or nil)

-- -- Значения из LSM:HashTable("border") добавляются с использованием цикла
-- for key, value in pairs(LSM:HashTable("border")) do
    -- core.CoreOptionsTable.args.borderTexture.values[key] = value
    -- print(key,value)
-- end
...
do
	...
	function core:BuildAboutMenu()
		...
		self.AboutOptionsTable.args.about.args.note = {
			type = "description",
			name = "|cffffcc33Note:|r |cff22bbff"..GetAddOnMetadata(folder, "X-Maintained").."|r",
			order = 20
		}
